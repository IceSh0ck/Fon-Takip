...
# requests kütüphanesi kaldırıldı
...
# GÜNCELLENDİ: Sadece 'stocks' alıyor, 'funds' kaldırıldı.
def _calculate_portfolio_return(stocks):
    ...
    # TEFAS (FON) HESAPLAMA DÖNGÜSÜ TAMAMEN KALDIRILDI
...
def save_portfolio():
    ...
    # GÜNCELLENDİ: 'funds' kontrolü kaldırıldı
    ...
    # GÜNCELLENDİ: 'funds' anahtarı kaldırıldı
    new_current_version = {
        'name': portfolio_name, 
        'fonTipi': fonTipi, 
...
def calculate():
    ...
    # GÜNCELLENDİ: Fonksiyon çağrısından 'funds' kaldırıldı
    result = _calculate_portfolio_return(stocks)
    return jsonify(result)

# GÜNCELLENDİ: Endpoint adı değiştirildi
@app.route('/get_all_portfolio_returns', methods=['GET'])
def get_all_portfolio_returns():
    """Tüm kayıtlı portföylerin günlük getirilerini hesaplar ve döndürür."""
    portfolios = load_portfolios()
    all_returns = []
...
# (Bu bölüm TEFAS çağırmadığı, sadece listeyi kaydettiği için değiştirilmedi)
...
def calculate_historical(portfolio_name):
    ...
    # TEFAS (FON) TARİHSEL VERİ ÇEKME DÖNGÜSÜ TAMAMEN KALDIRILDI
    ...
    # 'all_assets' artık sadece hisseleri içerdiği için bu bölüm doğru çalışacaktır
...
def calculate_dynamic_weights():
    ...
    # TEFAS (FON) FİYAT ÇEKME DÖNGÜSÜ TAMAMEN KALDIRILDI
    ...
    for asset in asset_market_values:
        ...
        # 'asset' artık sadece 'stock' tipi içerecek
        if asset['type'] == 'stock':
            hist = asset['data']
        ...
        # FON HESAPLAMA (elif asset['type'] == 'fund':) BLOĞU TAMAMEN KALDIRILDI
