...
# --- YENİ: YARDIMCI TEFAS FONKSİYONU ---
def _fetch_tefas_data(fund_code, sdt, fdt):
    """
    TEFAS API'sinden belirli bir fon kodunun verisini çeker.
    Engellenmemek için User-Agent başlığı ekler.
    Hata durumunda boş liste döndürür.
    """
    url = f"https://www.tefas.gov.tr/api/DB/BindHistoryPrice?sdt={sdt}&fdt={fdt}&kod={fund_code}"
    # GÜNCELLENDİ: TEFAS'ın engellememesi için tarayıcı başlığı eklendi.
    headers = {
    ...
    try:
        ...
        res.raise_for_status() # HTTP hatası varsa (4xx, 5xx) hata fırlat
        return res.json() # Başarılı olursa JSON verisini döndür
    except requests.exceptions.RequestException as e:
        print(f"TEFAS API hatası ({fund_code}): {e}")
...
        
        # GÜNCELLENDİ: TEFAS isteği, User-Agent başlığı eklenmiş yardımcı fonksiyona devredildi.
        try:
            # Önceki requests.get(...) satırları kaldırıldı.
            fund_data_raw = _fetch_tefas_data(fund_code, sdt, fdt)
            fund_data = [i for i in fund_data_raw if i.get('BirimPayDegeri') is not None]
...
        
        except Exception as e: # Veri işleme sırasında bir hata olursa (örn: tarih formatı)
            print(f"Fon ({fund_code}) işlenirken hata: {e}")
            asset_details.append({'type': 'fund', 'ticker': fund_code, 'daily_change': 0.0, 'weighted_impact': 0.0, 'error': 'Veri işlenemedi'})
            
...
        try:
            # Önceki requests.get(...) satırları kaldırıldı.
            fund_data = _fetch_tefas_data(fund_code, sdt_str, fdt_str)
            if fund_data: # Sadece veri varsa (boş liste değilse)
...
                asset_prices_df = pd.concat([asset_prices_df, df], axis=1)
        ...
        # Hata yakalama artık Pandas işlemleri için geçerli (veri alma _fetch_tefas_data içinde)
        except Exception as e: print(f"Tarihsel fon verisi işlenirken hata ({fund_code}): {e}")
...
            if not fund_data: raise Exception("Veri yok") # Hata bloğuna düşmek için
            
...
        except Exception as e: # _fetch_tefas_data'dan [] gelirse veya 'Veri yok'sa burası çalışır
            print(f"Dinamik ağırlık için fon fiyatı alınamadı ({fund_code}): {e}")
