...
# DÜZELTİLDİ: Bu fonksiyon artık tüm detayları geri döndürüyor.
...
def calculate():
    ...
    # Portföydeki toplam ağırlığı hesapla (eğer 100'den farklıysa normalize etmek için)
    total_weight = sum(float(s.get('weight', 0)) for s in stocks) + sum(float(f.get('weight', 0)) for f in funds)
    if total_weight == 0: total_weight = 100 # Sıfıra bölme hatasını önle
...
# DÜZELTİLDİ: Bu fonksiyon yeniden eklendi ve doğru çalışıyor.
...
def calculate_historical(portfolio_name):
    ...
    for i in range(30):
        ...
        # Tatil günlerinde veri olmayacağı için bir önceki mevcut güne bak
        d, current_prices_found = 1, False
        while d < 7:
            day_to_check = current_day - timedelta(days=d-1)
            prev_day_to_check = day_to_check - timedelta(days=d)
            
            day_return = 0.0
            valid_day = False

            # Hisse senetleri için o günkü değişimi bul
            for stock in stocks:
                weight = float(stock.get('weight', 0))
                ticker = stock.get('ticker').strip().upper()
                if ticker in stock_history_data:
                    hist_data = {k.date(): v for k, v in stock_history_data[ticker].items()}
                    current_price = hist_data.get(day_to_check)
                    prev_price = hist_data.get(prev_day_to_check)
                    if current_price and prev_price and prev_price > 0:
                        change = (current_price - prev_price) / prev_price * 100
                        day_return += (weight / total_weight) * change
                        valid_day = True
            
            # Fonlar için o günkü değişimi bul
            for fund in funds:
                weight = float(fund.get('weight', 0))
                fund_code = fund.get('ticker').strip().upper()
            ...
            if valid_day:
                # Daha önce bu tarih için bir getiri eklenmediyse ekle
                if not any(d['date'] == day_to_check.strftime('%d.%m.%Y') for d in daily_returns):
                    daily_returns.append({"date": day_to_check.strftime('%d.%m.%Y'), "return": f"{day_return:.2f}"})
                current_prices_found = True
                break
            d += 1
